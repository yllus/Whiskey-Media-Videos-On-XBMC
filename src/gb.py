...
class Main:
	...
	def addVideosFile(self, filt, mustPassAllFilters=False):
		...
		try:
			...
			print "addVideosFiles: Got {0} videos total to parse.".format(total)
	...
	def updateVideoList(self):
		...
		if self.__settings__.getSetting('ucat'):
			print "updateVideoList: Getting categories from {0}".format(settings_url)
		...
		total = list.getTotal()
		print "updateVideoList: Got {0} videos to update.".format(total)
		...
		try:
			...
			print "ERROR IN DATA FILE 2: {0}".format(e)
			offset = 0
